-- Short films use Seedance's reference-guided text-to-video routes. The
-- first/last-frame image-to-video routes are mutually exclusive with the
-- multi-reference contract and are no longer selectable in this product.
...
set video_model = case video_model
  when 'bytedance/seedance-2.0-fast/image-to-video' then 'bytedance/seedance-2.0-fast/text-to-video'
  else 'bytedance/seedance-2.5/text-to-video'
end;

update public.short_film_production_runs
set video_model = case video_model
  when 'bytedance/seedance-2.0-fast/image-to-video' then 'bytedance/seedance-2.0-fast/text-to-video'
  else 'bytedance/seedance-2.5/text-to-video'
end;

update public.short_film_automation_settings
set default_config = jsonb_set(
  coalesce(default_config, '{}'::jsonb),
  '{videoModel}',
  to_jsonb(case default_config->>'videoModel'
    when 'bytedance/seedance-2.0-fast/image-to-video' then 'bytedance/seedance-2.0-fast/text-to-video'
    else 'bytedance/seedance-2.5/text-to-video'
  end),
  true
...
-- The old continuity switch depended on extracting and resubmitting the prior
-- clip's last frame. Reference-guided scenes carry their own complete pack.
update public.video_plan_scenes set follows_previous=false where follows_previous;

alter table public.video_plans
  alter column video_model set default 'bytedance/seedance-2.5/text-to-video';
alter table public.video_plans
...
create or replace function public.enforce_short_film_scene_model_limits()
returns trigger language plpgsql set search_path=public as $$
declare selected_model text;
begin
  select video_model into selected_model from video_plans where id=new.video_plan_id;
  if selected_model='bytedance/seedance-2.0-fast/text-to-video' and new.duration_seconds>15 then
    raise exception 'SCENE_EXCEEDS_MODEL_DURATION';
  end if;
  if new.duration_seconds<4 or new.duration_seconds>30 then
    raise exception 'SCENE_EXCEEDS_MODEL_DURATION';
  end if;
  return new;
end $$;

drop trigger if exists enforce_short_film_scene_model_limits on public.video_plan_scenes;
create trigger enforce_short_film_scene_model_limits
before insert or update of duration_seconds,video_plan_id on public.video_plan_scenes
for each row execute function public.enforce_short_film_scene_model_limits();
...
declare v video_plans; s jsonb; sid uuid; seen uuid[]:='{}'; prior video_plan_scenes; ver integer; selected_model text;
begin
 ...
 selected_model:=coalesce(p_plan->>'video_model','bytedance/seedance-2.5/text-to-video');
 ...
 video_model=selected_model,audio_mode=p_plan->>'audio_mode',generate_audio=(p_plan->>'audio_mode')='native',caption=coalesce(p_plan->>'caption',''),
 ...
 for s in select value from jsonb_array_elements(p_scenes) loop
  ...
  if (s->>'duration_seconds')::integer < 4 or (s->>'duration_seconds')::integer > (case when selected_model='bytedance/seedance-2.0-fast/text-to-video' then 15 else 30 end) then raise exception 'SCENE_EXCEEDS_MODEL_DURATION'; end if;
  sid:=(s->>'id')::uuid;
...
comment on column public.video_plans.video_model is
  'Exact WaveSpeed reference-guided text-to-video model frozen for this plan revision.';
comment on column public.short_film_production_runs.video_model is
  'Exact WaveSpeed reference-guided text-to-video model frozen when the run is accepted.';
