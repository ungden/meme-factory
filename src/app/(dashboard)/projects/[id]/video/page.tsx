...
export default function VideoStudioPage() {
  ...
  const [resolution, setResolution] =
    useState<(typeof RESOLUTIONS)[number]>("720p");
  ...
  const draftRef = useRef<{ id: string | null; version: number | null }>({
    id: null,
    version: null,
  });
  ...
  const candidateImages = useMemo(
    () => [
      ...memes
        ...
        .map((m) => ({
          id: m.id,
          label: m.title || "Ảnh đã lưu",
          url: m.image_url!,
        })),
      ...characters.flatMap((c) => {
        const first = c.avatar_url || c.poses?.[0]?.image_url;
        return first
          ? [{ id: `character-${c.id}`, label: c.name, url: first }]
          : [];
      }),
    ],
    [characters, memes],
  ...
  const applyDraft = (draft: Record<string, unknown>) => {
    ...
    setLastImage(typeof draft.lastImage === "string" ? draft.lastImage : "");
    setReferences(
    ...
    setResolution(
      RESOLUTIONS.includes(draft.resolution as (typeof RESOLUTIONS)[number])
        ? (draft.resolution as (typeof RESOLUTIONS)[number])
        : "720p",
    );
    setAspect(
      ASPECTS.includes(draft.aspect as (typeof ASPECTS)[number])
        ? (draft.aspect as (typeof ASPECTS)[number])
        : "9:16",
    );
    setAudio(draft.audio !== false);
    setDialogue(typeof draft.dialogue === "string" ? draft.dialogue : "");
    setSpeaker(typeof draft.speaker === "string" ? draft.speaker : "");
  };
  ...
  async function assistVideo() {
    if (!prompt.trim()) {
      setError("Nhập ý tưởng trước để AI soạn video.");
      return;
    }
    setError("");
    setAssisting(true);
    try {
      const selectedCharacterIds = characters
        .filter((character) =>
          references.includes(
            character.avatar_url || character.poses?.[0]?.image_url || "",
          ),
        )
        .map((character) => character.id);
      const response = await fetch(
        `/api/projects/${projectRef}/creative-assists`,
        {
          method: "POST",
      ...
      if (!response.ok)
        throw new Error(json.error || "Không thể bắt đầu soạn video.");
      for (let attempt = 0; attempt < 30; attempt += 1) {
        const resultResponse = await fetch(
          `/api/projects/${projectRef}/creative-assists/${json.jobId}`,
          { cache: "no-store" },
        );
        const result = await resultResponse.json();
        if (!resultResponse.ok)
          throw new Error(result.error || "Không đọc được kết quả AI.");
        if (result.job.status === "completed") {
          setPrompt(result.job.result.prompt);
          setCaption(result.job.result.caption || "");
          setDialogue(result.job.result.dialogue || "");
          setSpeaker(result.job.result.speakerCharacterId || "");
          markEdited();
          return;
        }
        if (result.job.status === "failed")
          throw new Error(result.job.error?.code || "AI chưa soạn được video.");
        await new Promise((resolve) => window.setTimeout(resolve, 500));
      }
      throw new Error("AI đang xử lý lâu hơn bình thường.");
    } catch (reason) {
      setError(
        reason instanceof Error ? reason.message : "AI chưa soạn được video.",
      );
    } finally {
      setAssisting(false);
    }
  ...
  useEffect(() => {
    ...
    try {
      landing = JSON.parse(
        window.sessionStorage.getItem("aida:landing-draft") || "null",
      );
    } catch {
      /* ignore malformed landing draft */
    }
    ...
    if (
      characterName &&
      characterName !== "Thêm nhân vật mới" &&
      characters.length
    ) {
      const character = characters.find(
        (item) =>
          item.name.toLocaleLowerCase() === characterName.toLocaleLowerCase(),
      );
      const imageUrl =
        character?.avatar_url || character?.poses?.[0]?.image_url;
      if (imageUrl)
  ...
  useEffect(() => {
    const stored = window.sessionStorage.getItem(
      `aida:video-draft:${projectRef}`,
    );
    if (stored) {
      try {
        applyDraft(JSON.parse(stored) as Record<string, unknown>);
      } catch {
        window.sessionStorage.removeItem(`aida:video-draft:${projectRef}`);
      }
    }
    let active = true;
    fetch(`/api/projects/${projectRef}/drafts?tool=video`)
      ...
      .then((payload) => {
        ...
        if (payload?.draft) {
          ...
          if (payload.output) {
            ...
            if (
              payload.output.generation_job_id &&
              ["queued", "running"].includes(payload.output.status)
            )
              setJobId(payload.output.generation_job_id);
      ...
      .catch(() => {
        if (active) setDraftState("offline");
      })
      .finally(() => {
        if (active) {
          draftReadyRef.current = true;
          setDraftState((current) =>
            current === "offline" ? current : "saved",
          );
        }
      });
    return () => {
  ...
  useEffect(() => {
    if (
      ...
      (payloadKey === lastSavedPayloadRef.current &&
        output?.id === lastSavedOutputIdRef.current)
    )
      return;
        ...
        try {
          while (hasDraftContent) {
            ...
            const response = draft.id
              ? await fetch(`/api/projects/${projectRef}/drafts/${draft.id}`, {
                  method: "PUT",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
                    expected_version: draft.version,
                    payload: nextPayload,
                    ...(output ? { content_output_id: output.id } : {}),
                  }),
                })
              : await fetch(`/api/projects/${projectRef}/drafts`, {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
                    tool: "video",
            ...
            if (!response.ok)
              throw new Error(json.error || "Không thể lưu bản nháp.");
            draftRef.current = {
              id: json.draft.id,
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      if (!active || document.visibilityState === "hidden" || !navigator.onLine)
        return;
      try {
        const response = await fetch(`/api/continuity/jobs/${jobId}`, {
          cache: "no-store",
        });
        const job = await response.json();
        if (!response.ok)
          throw new Error(job.error || "Không đọc được tiến trình video.");
        if (job.phase === "dubbing_needs_review")
          setError(
            typeof job.error?.dubbing === "string"
              ? job.error.dubbing
              : "Lồng tiếng cần xem lại; clip gốc được giữ.",
          );
        if (job.contentOutput)
          setOutput((current) =>
            current ? { ...current, ...job.contentOutput } : job.contentOutput,
          );
  ...
  const config = () => ({
    ...
    dubbing: audio ? { text: dialogue, characterId: speaker } : null,
  });
  ...
  async function createOutput() {
    ...
    const setResponse = await fetch("/api/content-sets", {
      ...
      body: JSON.stringify({
        ...
        selected_character_ids: characters
          .filter((c) =>
            references.includes(c.avatar_url || c.poses?.[0]?.image_url || ""),
          )
          .map((c) => c.id),
      }),
    });
    ...
    if (!setResponse.ok)
      throw new Error(setJson.error || "Không tạo được bản nháp video.");
    const outputResponse = await fetch(
      `/api/content-sets/${setJson.contentSet.id}/outputs`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          kind: "video",
          format: mode === "text" ? aspect : "9:16",
    ...
    if (!outputResponse.ok)
      throw new Error(outputJson.error || "Không tạo được đầu ra video.");
  ...
  async function getQuote() {
    ...
    if (!prompt.trim() || (mode === "image" && !image)) {
      setError(
        mode === "image"
          ? "Chọn ảnh đầu trước khi báo giá."
          : "Nhập mô tả video trước khi báo giá.",
      );
      return;
    }
    setBusy(true);
    try {
      if (audio && (!dialogue.trim() || !speaker))
        throw new Error(
          "Nhập lời lồng tiếng và chọn người nói trước khi xem giá.",
        );
      const next = output ?? (await createOutput());
      setOutput(next);
      const response = await fetch(
        `/api/content-outputs/${next.id}/video/quote`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      const json = await response.json();
      if (!response.ok)
        throw new Error(json.error || "Không lấy được báo giá.");
      setQuote(json.quote);
    } catch (reason) {
      setError(
        reason instanceof Error ? reason.message : "Không lấy được báo giá.",
      );
    } finally {
      setBusy(false);
    }
  ...
  async function generate() {
    ...
    setBusy(true);
    try {
      const response = await fetch(`/api/content-outputs/${output.id}/video`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...config(), request_id: requestId() }),
      });
      const json = await response.json();
      if (!response.ok) throw new Error(json.error || "Không gửi được video.");
      setOutput((current) =>
      ...
      setError(
        reason instanceof Error ? reason.message : "Không gửi được video.",
      );
    } finally {
      setBusy(false);
    }
  ...
  const loading =
    projectLoading || charactersLoading || (shouldLoadMedia && memesLoading);
  if (!project && !projectLoading)
    return <div className="min-h-screen th-bg-primary" />;
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,0.92fr)_minmax(360px,1.08fr)]">
            ...
            >
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <button
                  onClick={assistVideo}
                  disabled={assisting || !prompt.trim()}
                  className="inline-flex h-9 items-center gap-2 rounded-lg border px-3 text-xs font-semibold th-text-accent th-bg-accent-light disabled:opacity-50"
                >
                  <Wand2 size={15} />
                  {assisting ? "AI đang soạn…" : "AI soạn video"}
                </button>
              </div>
              <textarea
                value={prompt}
              ...
              <label className="mt-3 block text-xs font-semibold th-text-secondary">
                ...
                <span className="font-normal th-text-tertiary">
                  · tách riêng khỏi lời thoại/video
                </span>
                <textarea
                  value={caption}
                  onChange={(event) => {
                  ...
                  className="mt-1.5 min-h-16 w-full rounded-lg border p-2 text-sm th-bg-input th-text-primary"
                  style={{ borderColor: "var(--border-primary)" }}
                  placeholder="AI sẽ đề xuất caption khi bạn bấm soạn video."
                />
              </label>
              {mode === "image" ? (
                <div className="mt-5">
                  ...
                  <p className="mt-1 text-xs th-text-tertiary">
                    Tỷ lệ video sẽ theo ảnh đầu.
                  </p>
                  <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">
                    {candidateImages.slice(0, 9).map((item) => (
                      <button
                        key={item.id}
                        onClick={() => {
                          markEdited();
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {characters.map((character) => {
                      const url =
                        character.avatar_url || character.poses?.[0]?.image_url;
                      if (!url) return null;
                      const active = references.includes(url);
                      return (
                        <button
                          key={character.id}
                          onClick={() => {
                            ...
                            setReferences(
                              active
                                ? references.filter((value) => value !== url)
                                : [...references, url],
                            );
                          }}
              ...
              {mode === "text" && (
                <div className="mt-5">
                  <p className="text-sm font-semibold th-text-primary">Tỷ lệ</p>
                  <div className="mt-2 flex gap-2">
                    {ASPECTS.map((value) => (
                      <button
                        key={value}
              ...
              <label className="mt-6 flex cursor-pointer items-center gap-3 text-sm th-text-primary">
                <input
                  checked={audio}
                  onChange={(event) => {
                    markEdited();
              ...
              {audio && (
                <div className="mt-3 space-y-2">
                  <select
                    aria-label="Người lồng tiếng"
                    className="w-full rounded-lg border p-2 th-bg-input th-text-primary"
                    value={speaker}
                    onChange={(e) => {
                      markEdited();
                  ...
                  >
                    <option value="">
                      Chọn người nói · dùng giọng đã duyệt
                    </option>
                    {characters.map((c) => (
                  ...
                  <textarea
                    aria-label="Lời lồng tiếng"
                    rows={3}
                    className="w-full rounded-lg border p-2 th-bg-input th-text-primary"
                    placeholder="Lời nói tiếng Việt…"
                    value={dialogue}
                    onChange={(e) => {
                      markEdited();
                  ...
                  <p className="text-xs th-text-secondary">
                    Clip đơn có một giọng đọc. Phim nhiều người đối đáp dùng Tạo
                    phim ngắn. Lồng tiếng không bảo đảm khớp môi.
                  </p>
                </div>
              )}
              {error && (
              ...
              <div
                className="sticky bottom-3 mt-6 rounded-xl border p-3 backdrop-blur"
                style={{
                  background:
              ...
              >
                ...
                >
                  ...
                  {quote
                    ? `Tạo video · ${quote.customerPoints.toLocaleString("vi-VN")} điểm`
                    : "Xem giá video"}
                </button>
                <p className="mt-2 text-center text-xs th-text-tertiary">
                  {draftState === "saving"
                    ? "Đang lưu bản nháp…"
                    : draftState === "offline"
            ...
            >
              ...
              {output?.media_url ? (
                <video
                  ...
                  src={`/api/content-outputs/${output.id}/media`}
                />
              ) : (
                <div
                  className="mt-5 flex aspect-[9/11] flex-col items-center justify-center rounded-2xl border border-dashed p-8 text-center"
                  style={{
                    borderColor: "var(--border-primary)",
