  ...
  return <div className="flex"><Sidebar projectId={projectRef} projectName={project?.name} />
    <main className="ml-0 min-h-screen flex-1 p-4 pt-16 md:ml-64 md:p-8 lg:p-10">
      <div className="mx-auto max-w-7xl">
        <header className="mb-6 flex flex-wrap items-end justify-between gap-4"><div><p className="mb-2 text-xs font-semibold uppercase tracking-[.16em] text-blue-500">Seedance 2.5</p><h1 className="text-3xl font-semibold tracking-tight th-text-primary">Tạo video</h1><p className="mt-2 text-sm th-text-tertiary">Tạo trực tiếp từ mô tả hoặc ảnh trong dự án. Kịch bản chỉ là hỗ trợ, không phải bước bắt buộc.</p></div><div className="flex gap-2"><Link className="rounded-xl border px-4 py-2 text-sm font-medium th-text-primary" href={`/projects/${projectRef}/video/multiscene`}>Video nhiều cảnh</Link><Link className="rounded-xl border px-4 py-2 text-sm font-medium th-text-primary" href={`/projects/${projectRef}/gallery`}>Mở thư viện</Link></div></header>
