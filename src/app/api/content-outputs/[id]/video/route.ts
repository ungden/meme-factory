...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (typeof body.prompt !== "string" || !body.prompt.trim() || (mode === "image" && typeof body.image !== "string") || ![5, 10, 15, 30].includes(Number(body.duration)) || !["720p", "1080p"].includes(body.resolution) || (mode === "text" && !["9:16", "1:1", "16:9"].includes(body.aspect_ratio ?? "9:16"))) return NextResponse.json({ error: "Cấu hình video không hợp lệ." }, { status: 400 });
  ...
  try {
    ...
    const quoteIsCurrent = !!savedQuote && !!output.quote_expires_at && new Date(output.quote_expires_at).getTime() > Date.now()
      && savedConfig?.mode === video.mode && savedConfig?.prompt === video.prompt && savedConfig?.image === video.image
      && savedConfig?.lastImage === video.lastImage && savedConfig?.aspectRatio === video.aspectRatio
      && JSON.stringify(savedConfig?.referenceImages ?? []) === JSON.stringify(video.referenceImages ?? [])
    ...
    const referenceManifest = [
      ...
      ...(video.lastImage ? [{ image: video.lastImage, role: "last_frame" }] : []),
      ...(video.referenceImages ?? []).map((image: string) => ({ image, role: "reference" })),
    ];
    const { data: job, error: jobError } = await getSupabaseAdmin().from("generation_jobs").insert({ project_id: projectId, content_set_id: output.content_set_id, content_output_id: id, creation_kind: "content_video", source_entity_type: "content_output", source_entity_id: id, workflow_version: "content-video-v2", provider: "wavespeed", model, status: "queued", compiled_prompt: video.prompt, reference_manifest: referenceManifest, manifest_hash: manifestHash, requested_output: { mode: video.mode, duration: video.duration, resolution: video.resolution, aspectRatio: video.mode === "text" ? video.aspectRatio : output.format, generateAudio: video.generateAudio }, estimated_points: quote.customerPoints, actual_points: quote.customerPoints, estimated_cost_usd: quote.providerCostUsd, created_by: user.id, checkpoint: { quote, requestId, quoteExpiresAt: output.quote_expires_at, video } }).select().single();
    if (jobError || !job) {
    ...
    const { data: deduction, error: deductError } = await getSupabaseAdmin().rpc("atomic_deduct_project_points", { _project_id: projectId, _actor_user_id: user.id, _cost: quote.customerPoints, _description: `Tạo video Seedance 2.5 ${video.duration}s ${video.resolution} (-${quote.customerPoints} điểm)`, _request_id: requestId, _ai_action: "video", _metadata: { model, mode: video.mode, duration: video.duration, resolution: video.resolution, content_output_id: id } });
    ...
    await getSupabaseAdmin().from("content_outputs").update({ generation_job_id: job.id, status: "queued", poster_url: video.image ?? null, script: video.prompt, duration_seconds: video.duration, source_snapshot: { mode: video.mode, image: video.image ?? null, lastImage: video.lastImage ?? null, referenceImages: video.referenceImages ?? [], aspectRatio: video.aspectRatio ?? null }, attempt_count: (output.attempt_count ?? 0) + 1 }).eq("id", id);
    let prediction;
    try {
