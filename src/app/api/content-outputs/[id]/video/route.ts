...
import { quoteSeedanceVideo, submitSeedanceVideo, WAVESPEED_SEEDANCE_MODEL } from "@/lib/wavespeed";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (output.generation_job_id) return NextResponse.json({ jobId: output.generation_job_id, outputId: id, duplicate: true }, { status: 202 });
  try {
    ...
    const quote = await quoteSeedanceVideo(video);
    ...
    await getSupabaseAdmin().from("content_outputs").update({ generation_job_id: job.id, status: "queued", poster_url: video.image, script: video.prompt, duration_seconds: video.duration }).eq("id", id).is("generation_job_id", null);
