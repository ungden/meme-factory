...
export async function checkProductionScript(
  ...
  return judge([
    {
      text: `Bạn kiểm duyệt kịch bản family catalogue trước khi hệ thống chi tiền sinh media. Chỉ passed khi câu chuyện rõ trong 2 giây đầu, có setup/payoff hợp lý, hành động khả thi theo tuổi/vóc dáng và quan hệ nhân vật, không làm nhục ai, không lặp mô-típ đã nêu trong dữ liệu, thoại tự nhiên và mọi người nói thuộc cast. Đặc biệt đánh dấu needs_review nếu trẻ nhỏ phải cõng/nâng trẻ lớn hơn, hành động nguy hiểm, hoặc cú chốt xuất hiện ngẫu nhiên. Không sửa kịch bản.\nHỒ SƠ: ${JSON.stringify(profile)}\nKỊCH BẢN: ${JSON.stringify(story)}\nCẢNH: ${JSON.stringify(plan.video_plan_scenes.map((s) => ({ speaker: s.speaker_character_id, dialogue: s.dialogue, action: s.action, cast: s.cast_snapshot.map((c) => c.characterId) })))}`,
    },
  ]);
}
