...
export async function checkVisualTask(
  ...
  try {
    const parts: Array<Record<string, unknown>> = [
      {
        text: `Kiểm tra media của một shot phim theo đúng phong cách ảnh chuẩn. Media đầu tiên là kết quả; các ảnh sau là ảnh chuẩn từng nhân vật. Với kind image, đây chỉ là KHUNG ĐẦU trước hành động: chỉ kiểm tra đúng bối cảnh, cast cần có, nhận diện, trang phục, đạo cụ và tư thế mở; tuyệt đối không đòi ảnh tĩnh thể hiện chuyển động, khóc, ôm, nói, khớp môi hoặc kết quả ở các beat sau. Với kind video và audioMode dubbed, đây là chuyển động im tiếng trước lồng tiếng: chỉ kiểm tra hình và đúng người diễn từng lượt, không đòi audio. Với kind dub, bắt buộc nghe/xem đúng người nói, kiểm tra lệch môi; ghép audio thành công không chứng minh khớp môi. Chỉ passed khi đúng số người và đúng nhận diện, khuôn mặt/tóc/trang phục không trôi, không thêm người/chữ/lưới và hình không lỗi. Với video có storyboard, kiểm tra từng lượt theo thứ tự: chỉ đúng người nói cử động môi; người khác phản ứng không lời. Mốc beat là dự kiến, không dùng làm bằng chứng audio thực. Cận cảnh có thể chỉ hiện người nói; khung mở phải có đủ cast. Nếu không đủ bằng chứng để xác định người nói, miệng người khác chuyển động như đang nói, hoặc video chỉ là ảnh ghép tĩnh thì needs_review. Không suy đoán và không dùng kịch bản dự kiến thay cho bằng chứng nghe/nhìn.\nTASK: ${JSON.stringify({ audioMode: task.input.audioMode, kind: task.kind, cast: task.input.cast, dialogue: task.input.dialogue, speakerCharacterId: task.input.speakerCharacterId, setting: task.input.setting, storyboard: task.input.storyboard })}`,
      },
