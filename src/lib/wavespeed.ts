...
export const WAVESPEED_SEEDANCE_IMAGE_TO_VIDEO_MODEL = "bytedance/seedance-2.5/image-to-video";
export const WAVESPEED_SEEDANCE_TEXT_TO_VIDEO_MODEL = "bytedance/seedance-2.5/text-to-video";
...
export function getSeedanceModel(input: Pick<VideoRequest, "mode">) {
  return input.mode === "text" ? WAVESPEED_SEEDANCE_TEXT_TO_VIDEO_MODEL : WAVESPEED_SEEDANCE_IMAGE_TO_VIDEO_MODEL;
}
...
function providerInputs(input: VideoRequest) {
  ...
  if (!input.image) throw new Error("Ảnh đầu là bắt buộc khi tạo video từ ảnh.");
  return { ...common, image: input.image, ...(input.lastImage ? { last_image: input.lastImage } : {}) };
}
...
export async function quoteSeedanceVideo(input: VideoRequest) {
  const data = await request("/model/price", {
    method: "POST",
    body: JSON.stringify({ model_id: getSeedanceModel(input), inputs: providerInputs(input) }),
  });
...
export async function submitSeedanceVideo(input: VideoRequest, webhook: string) {
  const endpoint = input.mode === "text" ? "/bytedance/seedance-2.5/text-to-video" : "/bytedance/seedance-2.5/image-to-video";
