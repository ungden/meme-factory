...
export function isSceneDuration(
  value: unknown,
): value is (typeof MULTISCENE_DURATIONS)[number] {
  return (
    Number.isInteger(Number(value)) && Number(value) >= 4 && Number(value) <= 30
  );
}
  ...
  return {
    ...
    dialogue:
      typeof scene.dialogue === "string"
        ...
        : "",
    action:
      typeof scene.action === "string" ? scene.action.trim().slice(0, 900) : "",
    setting:
    ...
    characterIds: Array.isArray(scene.characterIds)
      ...
      : [],
    durationSeconds: isSceneDuration(scene.durationSeconds)
      ? scene.durationSeconds
      : 5,
    startImageUrl:
      typeof scene.startImageUrl === "string" && scene.startImageUrl
        ? scene.startImageUrl
        : null,
    endImageUrl:
    ...
    motionPrompt:
      typeof scene.motionPrompt === "string"
        ? scene.motionPrompt.trim().slice(0, 1600)
        : "",
    camera:
      typeof scene.camera === "string" ? scene.camera.trim().slice(0, 600) : "",
    sourceMode: scene.sourceMode === "ai" ? "ai" : "manual",
  };
}

export function buildScenePrompt(
  scene: Pick<SceneInput, "dialogue" | "action" | "setting">,
  cast: SceneCast[],
  ...
  const identities = cast
    .map(
      (item) =>
        `${item.name} [phiên bản đã khoá ${item.assetVersionId ?? "legacy"}]: ${item.description || item.personality || "nhân vật 3D đã duyệt"}`,
    )
    .join("; ");
  return [
    ...
    speaking
      ? `${speaking.name} là người nói duy nhất trong cảnh.`
      : "Không có người nói được chỉ định.",
    scene.dialogue
      ? `Lời thoại tiếng Việt phải nói trọn câu, tự nhiên, không đổi người nói: “${scene.dialogue}”.`
      : "Không có lời thoại.",
...
export function sceneVideoRequest(
  scene: {
    prompt: string;
...
export function totalSceneDuration(
  scenes: Array<{ duration_seconds?: number | null; durationSeconds?: number }>,
) {
  return scenes.reduce(
    (total, scene) =>
      total + Number(scene.duration_seconds ?? scene.durationSeconds ?? 0),
    0,
  );
}
